...
pub fn validate_raw_log(raw: &str) -> Result<&str, SecurityError> {
    ...
    // [ATLATL-ORDNANCE] Stage 2 Aggressive Patterns
    // Note: We search for both literal escape sequences and common keywords
    const PATTERNS: &[(&[u8], &str)] = &[
        (b"\\x90\\x90\\x90", "nop_sled_literal"),
        (b"\\xEB\\xFE", "jmp_self_literal"),
...
mod tests {
    ...
    fn test_malicious_patterns() {
        ...
        // Using literal text representation to avoid Rust hex escape range errors in tests
