...
//!
//! [ATLATL-ORDNANCE] Version 9.0.0-XOCHIMILCO: Spectral Mesh & Adversarial Protection
...
pub fn detect_xochimilco_adversarial(event: &KalpixkEvent) -> NodeResult {
    ...
    if raw.contains("xochimilco")
        || raw.contains("adversarial_drift")
        || raw.contains("tensor_poison")
    {
        score += 0.95;
        techniques.push("T1565".to_string());
    }

    if event.source_type == "xochimilco_strike" {
        score = 1.0;
    ...
    NodeResult {
        ...
        level: SeverityScore::new(score).as_level(),
        mitre_techniques: techniques,
        description: "v9.0.0-XOCHIMILCO Adversarial Model Poisoning & Tensor Drift Shield"
            .to_string(),
    }
}
