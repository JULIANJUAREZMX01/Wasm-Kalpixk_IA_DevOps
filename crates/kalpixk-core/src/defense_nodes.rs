...
pub fn detect_embedded_tampering(event: &KalpixkEvent) -> NodeResult {
    let mut score = 0.0;
    let mut techniques = Vec::new();
    let raw = event.raw.to_lowercase();
    ...
    if raw.contains("jtag")
        ...
        || raw.contains("spidev")
        || raw.contains("flash_dump")
        || raw.contains("bus_injection")
    ...
    if raw.contains("embedded_probe") || raw.contains("edge_poisoning") {
        score += 0.90;
        techniques.push("T1565".to_string());
    }

    if event.source_type == "embedded_tamper" || event.source_type == "edge_probe" {
        score = 1.0;
    ...
    NodeResult {
        ...
        level: SeverityScore::new(score).as_level(),
        mitre_techniques: techniques,
        description: "Detection of physical probing, hardware tampering, and edge telemetry poisoning on decentralized embedded nodes"
            .to_string(),
    }
}
...
pub fn analyze_all_nodes(event: &KalpixkEvent) -> Vec<NodeResult> {
    ...
    vec![
        ...
        detect_embedded_tampering(event),
    ]
}
