...
//! - Node-10: EMBEDDED_NODE_DEFENDER (v10.0-ATLATL)
//!
//! [ATLATL-ORDNANCE] Version 10.0: Decentralized Embedded Defense Nodes
...
mod tests {
    ...
    #[test]
    fn test_embedded_node_defender_detects_tamper() {
        let event = KalpixkEvent {
            timestamp_ms: 1000,
            event_type: EventType::Unknown,
            local_severity: 0.8,
            source: "embedded-node-01".to_string(),
            destination: None,
            user: None,
            process: None,
            metadata: HashMap::new(),
            raw: "hardware_probing detected on UART interface".to_string(),
            source_type: "embedded_probe".to_string(),
            fingerprint: "fp123".to_string(),
        };

        let result = detect_embedded_node_defender(&event);
    ...
    #[test]
    fn test_embedded_node_defender_clean_event() {
        let event = KalpixkEvent {
            timestamp_ms: 1000,
            event_type: EventType::LoginSuccess,
            local_severity: 0.1,
            source: "node-01".to_string(),
            destination: None,
            user: Some("admin".to_string()),
            process: None,
            metadata: HashMap::new(),
            raw: "normal operating syslog log".to_string(),
            source_type: "syslog".to_string(),
            fingerprint: "fp456".to_string(),
        };

        let result = detect_embedded_node_defender(&event);
        assert_eq!(result.score, 0.0);
        assert_eq!(result.level, SeverityLevel::Clean);
    }
}
...
pub fn detect_embedded_node_defender(event: &KalpixkEvent) -> NodeResult {
    ...
    if raw.contains("embedded_tamper")
        || raw.contains("hardware_probing")
        || raw.contains("side_channel")
        || raw.contains("uart_intercept")
    {
        score += 0.95;
        techniques.push("T1200".to_string());
    }

    if event.source_type == "embedded_defense_node" || event.source_type == "embedded_probe" {
        score = 1.0;
        techniques.push("T1056".to_string());
    }
    ...
    NodeResult {
        ...
        level: SeverityScore::new(score).as_level(),
        mitre_techniques: techniques,
        description: "Detection of probing, physical tampering, and side-channel analysis against decentralized embedded nodes"
            .to_string(),
    }
}
