...
pub fn detect_embedded_node_tampering(event: &KalpixkEvent) -> NodeResult {
    ...
    if event.source_type == "embedded_node_telemetry" && raw.contains("unauthorized_access") {
        score = 1.0;
...
mod tests {
    ...
    fn test_detect_embedded_node_tampering() {
        let event = KalpixkEvent {
            fingerprint: "test-emb-1".to_string(),
            timestamp_ms: 1000,
            event_type: EventType::Unknown,
            local_severity: 0.8,
            source: "10.0.42.1".to_string(),
            source_type: "embedded_node_telemetry".to_string(),
            user: None,
            destination: None,
            process: None,
