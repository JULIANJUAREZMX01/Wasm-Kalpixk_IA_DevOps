...
//!
//! 9 nodes for detecting Red Team techniques:
...
//!
//! [ATLATL-ORDNANCE] Version 9.0.0-XOCHIMILCO: Mesh Coordination & Adversarial Shield
...
pub fn detect_xochimilco_adversarial(event: &KalpixkEvent) -> NodeResult {
    ...
    if raw.contains("drift_guard") || raw.contains("xochimilco") || raw.contains("poisoning") {
        score += 0.85;
        techniques.push("T1565".to_string());
    }

    if event.source_type == "xochimilco_strike" || event.source_type == "v9_strike" {
        score = 1.0;
        techniques.push("T1565.001".to_string());
    }
    ...
    NodeResult {
        ...
        level: SeverityScore::new(score).as_level(),
        mitre_techniques: techniques,
        description: "Node-9: Detection of adversarial poisoning attempts against ensemble models"
            .to_string(),
    }
}
