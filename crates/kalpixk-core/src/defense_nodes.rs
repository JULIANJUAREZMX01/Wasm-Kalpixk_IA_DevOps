...
pub fn detect_mesh_auth(event: &KalpixkEvent) -> NodeResult {
    let mut score = 0.0;
    let mut techniques = Vec::new();
    if event.source_type == "mesh_sync" && !event.metadata.contains_key("x_xochi_auth") {
        score = 1.0;
        techniques.push("T1557".to_string());
    }
    NodeResult {
        ...
        level: SeverityScore::new(score).as_level(),
        mitre_techniques: techniques,
        description: "Xochimilco Mesh Mutual Authentication validation".to_string(),
    }
}
...
pub fn detect_integrity_guard(event: &KalpixkEvent) -> NodeResult {
    ...
    if raw.contains("integrity_violation") || raw.contains("hook_detected") {
        score = 1.0;
