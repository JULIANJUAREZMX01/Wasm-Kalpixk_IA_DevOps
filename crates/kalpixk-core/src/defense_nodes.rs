...
//! - Node-8: GUERRILLA (v8.0.0)
//!
//! [ATLATL-ORDNANCE] Version 8.0: Guerrilla Counter-Strike Coordination
...
pub fn detect_guerrilla_threat(event: &KalpixkEvent, raw_lower: &str) -> NodeResult {
    ...
    if event.local_severity > 0.95 {
        score += 0.4; // Synergistic threat
    }
    ...
    NodeResult {
        ...
        level: SeverityScore::new(score).as_level(),
        mitre_techniques: techniques,
        description: "Stage 8 Guerrilla retaliation detection".to_string(),
    }
}
