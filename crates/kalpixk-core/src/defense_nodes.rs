...
//!
//! [ATLATL-ORDNANCE] Version 9.0: XOCHIMILCO Spectral Mesh Coordination
...
pub fn detect_xochimilco_threat(event: &KalpixkEvent) -> NodeResult {
    let mut score = 0.0;
    let mut techniques = Vec::new();
    let raw = event.raw.to_lowercase();

    if raw.contains("xochimilco") || raw.contains("spectral_mesh") {
        score += 0.95;
        techniques.push("TA-XOCHIMILCO-V9".to_string());
    }

    if event.source_type == "v9_xochimilco_strike" {
        score = 1.0;
        techniques.push("TA-XOCHIMILCO-V9".to_string());
    }
    ...
    NodeResult {
        ...
        level: SeverityScore::new(score).as_level(),
        mitre_techniques: techniques,
        description: "Coordination of Stage 9 hardening against spectral threats".to_string(),
    }
}
...
pub fn analyze_all_nodes(event: &KalpixkEvent) -> Vec<NodeResult> {
    ...
    vec![
        ...
        detect_xochimilco_threat(event),
    ]
}
