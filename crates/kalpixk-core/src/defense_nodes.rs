...
//!
//! [ATLATL-ORDNANCE] Version 9.0: Xochimilco Mesh Protection
...
pub fn detect_mesh_auth(event: &KalpixkEvent) -> NodeResult {
    let mut score = 0.0;
    if event.source_type == "mesh_sync" && !event.metadata.contains_key("x_kalpixk_mesh_token") {
        score = 1.0;
    }
    NodeResult {
        ...
        level: SeverityScore::new(score).as_level(),
        mitre_techniques: vec!["T1557".to_string()],
        description: "Mutual mesh authentication validation".to_string(),
    }
}
