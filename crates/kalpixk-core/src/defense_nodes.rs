...
pub fn detect_embedded_node_defender(event: &KalpixkEvent) -> NodeResult {
    ...
    if raw.contains("embedded_tamper")
        || raw.contains("firmware_probe")
        || raw.contains("side_channel_attack")
    ...
    NodeResult {
        ...
        level: SeverityScore::new(score).as_level(),
        mitre_techniques: techniques,
        description: "Detection of probing, tampering, and firmware interference against decentralized embedded nodes"
            .to_string(),
    }
}
...
mod tests {
    ...
    fn test_detect_embedded_node_defender() {
        let event = KalpixkEvent {
            raw: "WARNING: JTAG_INTRUSION attempt on embedded node".to_string(),
        ...
        let all_results = analyze_all_nodes(&event);
