...
#[wasm_bindgen]
pub fn v9_xochimilco_active_memory_scrambling_wasm(target: &mut [u8], seed: u64) {
    #[cfg(target_arch = "wasm32")]
    unsafe {
        v9_xochimilco_active_memory_scrambling(target.as_mut_ptr(), target.len(), seed);
    }
    #[cfg(not(target_arch = "wasm32"))]
    motor::v9_xochimilco_active_memory_scrambling(target, seed);
}
...
pub fn process_batch(logs_json: &str, source_type: &str) -> String {
    ...
    if lines.len() > 10 {
        ...
        // [ATLATL-ORDNANCE] v9 XOCHIMILCO Hardening
        #[cfg(target_arch = "wasm32")]
        unsafe {
            v9_xochimilco_jit_shield(decoy_buffer.as_mut_ptr(), decoy_buffer.len(), seed ^ 0x90C);
