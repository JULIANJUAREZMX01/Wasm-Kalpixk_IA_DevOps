...
// Security functions are now called directly from motor.rs
...
pub fn process_batch(logs_json: &str, source_type: &str) -> String {
    ...
    if lines.len() > 10 {
        ...
        motor::v9_active_memory_scrambling(&mut decoy_buffer, seed ^ 0x90C);
        motor::v9_chaotic_interleaving(&mut decoy_buffer, 31);
