...
## 2026-09-05 - [Uncaught Variable Reference in API Result Processing]
**Vulnerability:** Undefined variable access (`threshold` instead of `adaptive_threshold`) inside `/api/detect` batch anomaly loop.
**Learning:** Intercepted exceptions inside request handlers can cause unhandled 500 status responses leading to Denial of Service (DoS) for batch log ingestion endpoints.
**Prevention:** Ensure strict typing and endpoint regression tests covering batch feature inputs against ensemble scoring.
