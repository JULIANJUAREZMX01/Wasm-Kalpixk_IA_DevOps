...
## 2026-09-12 - [Array Vectorization in Drift Guard]
**Learning:** Found an unnecessary O(n) Python-level `for` loop during threshold calculations in `AdversarialDriftGuard.update()` when handling batches of scores. This degraded inference pipeline performance. Additionally, tests generating binary model files during execution were incorrectly staging these files in git.
**Action:** Always prefer native NumPy vectorized operations like boolean masking (e.g., `scores_arr[scores_arr < threshold]`) over list iterations for ML components, and always verify and restore any binary model files auto-regenerated by test suites before committing.
