...
## 2026-05-26 - [React List Rendering Optimization for Derived Components]
**Learning:** In `Dashboard.tsx`, rendering an array of primitive strings via `.map()` inline (e.g. `terminalOutput.map`) caused each item to execute a new state initialization like `fmt(new Date())` whenever the parent re-rendered. This not only degraded performance due to O(N) recalculations for un-changed lines, but it caused a functional bug where every terminal line visually reset to the *current* time instead of its *creation* time. Additionally, inline `.filter` inside derived tabs like `WarRoomTab` triggered redundant O(N) evaluations every render.
**Action:** Always wrap derived components mapping over simple primitives into their own `React.memo` components, using the unique content or index+content as a stable key, and always cache heavy array filtering inside derived tabs with `useMemo`.
