...
export default function Dashboard() {
  ...
  const [terminalOutput, setTerminalOutput] = useState<string[]>(["[SYSTEM] ATLATL-ORDNANCE v9.0.0-XOCHIMILCO initialized.", "[SYSTEM] Mesh operating in GHOST MODE v9.", "[SYSTEM] Node-9 (MESH_AUTH) & Node-10 (INTEGRITY_GUARD) synchronized.", "[SYSTEM] Awaiting aggressor vectors..."]);
...
const RealtimeTab = React.memo(function RealtimeTab({ chart, terminalOutput }: { chart: { t: number; s: number }[], terminalOutput: string[] }) {
  ...
  const NODES = [
    ...
    { id: "MESH_AUTH",desc: "Node-9 · Auth v9",   load: 5,  port: 8087, status: "ACTIVE" },
    { id: "INTEGRITY",desc: "Node-10 · Guard v9", load: 2,  port: 8088, status: "ACTIVE" },
  ];
