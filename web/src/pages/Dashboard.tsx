...
export default function Dashboard() {
  ...
  const [terminalOutput, setTerminalOutput] = useState<string[]>(["[SYSTEM] ATLATL-ORDNANCE v9.0.0-XOCHIMILCO initialized.", "[SYSTEM] XOCHIMILCO MODE v9: ACTIVE.", "[SYSTEM] Mesh Auth N9 & Integrity Guard N10: ENGAGED.", "[SYSTEM] Awaiting aggressor vectors..."]);
            ...
            <button
              onClick={async () => {
                try {
                  ...
                  alert("v9 XOCHIMILCO GUILLOTINE ENGAGED: SYSTEMIC COLLAPSE INITIATED");
                } catch (e) {
...
const RealtimeTab = React.memo(function RealtimeTab({ chart, terminalOutput }: { chart: { t: number; s: number }[], terminalOutput: string[] }) {
  ...
  const NODES = [
    ...
    { id: "MESH_AUTH",desc: "Node-9 Auth",        load: 2,  port: 8083, status: "ACTIVE" },
    { id: "INT_GUARD",desc: "Node-10 Integrity", load: 1,  port: 8084, status: "ACTIVE" },
