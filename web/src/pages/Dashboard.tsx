...
const WarRoomTab = React.memo(function WarRoomTab({ terminalOutput }: { terminalOutput: string[] }) {
  const alerts = useAlertStore((s) => s.alerts);
  const strikes = React.useMemo(() => terminalOutput.filter(l => l.includes("[STRIKE]") || l.includes("[GUILLOTINE]")), [terminalOutput]);
  const targetedAlerts = React.useMemo(() => alerts.filter(a => a.score > 0.8).slice(0, 5), [alerts]);
