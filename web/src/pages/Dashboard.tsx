...
const RealtimeTab = React.memo(function RealtimeTab({ chart, terminalOutput }: { chart: { t: number; s: number }[], terminalOutput: string[] }) {
  ...
  const NODES = [
    ...
    { id: "MESH_AUTH",desc: "Node-9 Auth Guard",  load: 5,  port: 8087, status: "ACTIVE" },
    { id: "INTG_GUARD",desc: "Node-10 Binary",    load: 12, port: 8088, status: "ACTIVE" },
              ...
              {strikes.length === 0 ? (
                ...
                strikes.map((s, i) => (
                  <div key={i} style={{
                    ...
                    <div style={{ color: T.dim, fontSize: 9 }}>IMPACT: TOTAL_SYSTEMIC_COLLAPSE (100GB/s)</div>
