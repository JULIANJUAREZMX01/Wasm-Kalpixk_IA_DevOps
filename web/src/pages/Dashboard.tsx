...
export default function Dashboard() {
  ...
  const [terminalOutput, setTerminalOutput] = useState<string[]>(["[SYSTEM] ATLATL-ORDNANCE v9.0.0-XOCHIMILCO initialized.", "[SYSTEM] Mesh operating in GHOST MODE v9.", "[SYSTEM] XOCHIMILCO MODE v9: ACTIVE.", "[SYSTEM] Awaiting aggressor vectors..."]);
...
const RealtimeTab = React.memo(function RealtimeTab({ chart, terminalOutput }: { chart: { t: number; s: number }[], terminalOutput: string[] }) {
  ...
  const NODES = [
    ...
    { id: "NODE-9",   desc: "Mesh Auth",          load: 2,  port: 8087, status: "ACTIVE" },
    { id: "NODE-10",  desc: "Binary Integrity",   load: 1,  port: 8088, status: "ACTIVE" },
