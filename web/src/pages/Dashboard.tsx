...
function Label({ text, accent = T.amber }: { text: string; accent?: string }) {
  return (
    <div
      style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8, flexShrink: 0 }}
    >
      <div
        style={{ width: 5, height: 5, background: accent, transform: "rotate(45deg)" }}
      />
...
function Bar({ pct, color }: { pct: number; color: string }) {
  return (
    <div
      style={{ height: 3, background: T.border, borderRadius: 2, overflow: "hidden", flex: 1 }}
    >
      <div
        style={{
...
export default function Dashboard() {
  ...
  const threatLevel =
    alerts[0]?.score >= 0.85 ? "CRITICAL" : alerts[0]?.score >= 0.65 ? "HIGH" : "NORMAL";
  const tlColor =
    threatLevel === "CRITICAL" ? T.red : threatLevel === "HIGH" ? T.amber : T.green;
  ...
  return (
    ...
    >
        ...
        <div style={{ display: "flex", gap: 20, alignItems: "center" }}>
          {[
            ...
            <div key={l} style={{ textAlign: "right" }}>
              ...
              <div style={{ color: c, fontSize: 15, fontWeight: 700, lineHeight: 1.1 }}>
                {v}
              </div>
            </div>
          ))}
      ...
      >
        <span style={{ color: T.dim, letterSpacing: 2, whiteSpace: "nowrap" }}>
          ▸ TRANSCRIPT
        </span>
        {alerts.slice(0, 3).map((a, i) => (
          <span
            key={a.id}
            style={{ color: i === 0 ? T.green : T.dim, whiteSpace: "nowrap" }}
          >
            [{fmt(a.ts)}] SENTINEL.AUDIT score={a.score.toFixed(3)} src={a.ip} type={
              a.eventType
            }
          </span>
        ))}
        <span
          style={{ marginLeft: "auto", color: T.dim, whiteSpace: "nowrap", letterSpacing: 1 }}
        >
        ...
        <div>
          ...
          {NODES.map((n) => (
            ...
            >
              ...
              >
                <span style={{ color: nodeColor(n.status), fontWeight: 700, fontSize: 11 }}>
                  {n.id}
                </span>
                <span
                  style={{
                    ...
                    background: `${nodeColor(n.status)}11`,
              ...
              <div style={{ color: T.dim, fontSize: 9, marginBottom: 4 }}>
                {n.desc} :{n.port}
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                <Bar
                  pct={n.load}
                  color={n.load > 70 ? T.red : n.load > 45 ? T.amber : T.green}
                />
                <span style={{ color: T.dim, fontSize: 9, width: 28, textAlign: "right" }}>
                  {n.load}%
                </span>
              </div>
            </div>
            ...
            <div
              ...
              style={{
                ...
                borderLeft: `3px solid ${
                  hp.hits > 20 ? T.red : hp.hits > 0 ? T.amber : T.dim
                }`,
                transition: "all .5s",
              }}
            >
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                ...
                <span
                  style={{
                    color: hp.hits > 20 ? T.red : hp.hits > 0 ? T.amber : T.dim,
                    fontWeight: 700,
                    fontSize: 11,
                  }}
                >
      ...
      >
        ...
        >
          ...
          <div
            style={{
              display: "grid",
              gridTemplateColumns: "52px 110px 28px 1fr 80px",
              gap: 6,
              padding: "0 6px 5px",
              color: T.dim,
              fontSize: 9,
            ...
            {alerts.map((a, i) => (
              ...
              >
                ...
                <div style={{ textAlign: "right" }}>
                  ...
                  <div
                    style={{ color: scoreColor(a.score), fontSize: 12, fontWeight: 700 }}
                  >
      ...
      <div style={{ background: T.panel, overflow: "auto", padding: 10 }}>
        ...
        >
          {[
            {
              l: "VRAM",
              v: `${metrics.vramUsedGb}/${metrics.vramTotalGb} GB`,
              pct: (metrics.vramUsedGb / metrics.vramTotalGb) * 100,
              c: T.green,
            },
            {
              l: "GPU LOAD",
              v: `${metrics.gpuLoadPct}%`,
              pct: metrics.gpuLoadPct,
              c: metrics.gpuLoadPct > 70 ? T.red : T.amber,
            },
            { l: "LATENCY", v: `${metrics.gpuLatencyMs}ms`, pct: metrics.gpuLatencyMs, c: T.green },
            {
              l: "F1 SCORE",
              v: metrics.f1Score.toFixed(3),
              pct: metrics.f1Score * 100,
              c: T.green,
            },
          ].map((r) => (
...
function ParsersTab() {
  const [input, setInput] = useState(
    "Apr 04 03:22:11 cedis sshd[1234]: Failed password for root from 185.220.101.42 port 44321 ssh2"
  );
  ...
  const parse = () => {
    ...
    const demo: Record<string, unknown> = {
      ...
      local_severity: input.toLowerCase().includes("drop")
        ? 0.85
        : input.toLowerCase().includes("failed")
        ? 0.45
        : 0.3,
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 4 }}>
            {[
              ...
              <div key={f} style={{ fontSize: 9, color: T.dim, padding: "2px 0" }}>
                <span style={{ color: T.amber }}>F{i.toString().padStart(2, "0")}</span> {f}
              </div>
            ))}
          </div>
        </div>
...
function BenchmarkTab() {
  ...
  const rows = [
    ...
    {
      metric: "Latency 100 events",
      cpu: "182 ms",
      gpu: `${metrics.gpuLatencyMs} ms`,
      speedup: "5.4x",
      target: "<50ms ✅",
    },
    ...
    {
      metric: "WASM module size",
      cpu: "—",
      gpu: `${metrics.wasmSizeKb} KB`,
      speedup: "—",
      target: "<1MB ✅",
    },
    {
      metric: "VRAM used",
      cpu: "—",
      gpu: `${metrics.vramUsedGb}/${metrics.vramTotalGb} GB`,
      speedup: "—",
      target: "7.4% ✅",
    },
    {
      metric: "LLM cost/explanation",
      cpu: "$0.02",
      gpu: "$0.00",
      speedup: "∞",
      target: "$0 ✅",
  ...
  return (
    <div style={{ flex: 1, padding: 20, overflow: "auto", background: T.panel }}>
      <div style={{ maxWidth: 860, margin: "0 auto" }}>
          ...
          <tbody>
            {rows.map((r, i) => (
              ...
              >
                ...
                <td style={{ padding: "8px 10px", color: T.green, fontWeight: 700 }}>
                  {r.gpu}
                </td>
                <td style={{ padding: "8px 10px", color: T.amber, fontWeight: 700 }}>
                  {r.speedup}
                </td>
        ...
        >
          <div style={{ color: T.amber, fontSize: 9, letterSpacing: 2, marginBottom: 6 }}>
            ⚠ NOTA SOBRE SPEEDUP
          </div>
          <p style={{ color: T.text, fontSize: 10, lineHeight: 1.7 }}>
            El speedup actual de <strong style={{ color: T.amber }}>3.6x</strong> corresponde al
            dataset sintético pequeño. Con el dataset CERT de 100K eventos en el droplet AMD
            MI300X real, el speedup esperado es{" "}
            <strong style={{ color: T.green }}>15–23x</strong> (baseline de literatura para
            cuML vs sklearn con N≥100K). Benchmark completo pendiente de acceso al droplet AMD
            Developer Cloud.
          </p>
          <div
            style={{
          ...
          >
            python training/train_models.py --dataset synthetic --n-samples 100000 --device auto
            --benchmark
          </div>
        </div>
      </div>
...
function MitreTab() {
  const TECHNIQUES = [
    ...
    {
      id: "T1048",
      name: "Exfiltration Over Alt Chnl",
      tactic: "Exfiltration",
      count: 3,
      severity: 0.91,
    },
              ...
              style={{
                ...
                background:
                  t.severity >= 0.85
                    ...
                    : T.surface,
                border: `1px solid ${
                  t.severity >= 0.85 ? T.red : t.severity >= 0.7 ? T.amber : T.border
                }`,
