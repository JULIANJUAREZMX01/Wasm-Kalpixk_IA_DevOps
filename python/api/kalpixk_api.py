...
async def analyze(request: Request, req: LogRequest, api_key: str = Depends(verify_api_key)):
    ...
    if len(features[0]) != 32:
        raise HTTPException(422, f"Se esperan 32 features, recibidas: {len(features[0])}")

    t0 = time.time()
    features_array = torch.from_numpy(np.array(features, dtype=np.float32)).to(_device)
    scores, techniques, confidences = ens.predict(features_array)
    score = scores[0]
