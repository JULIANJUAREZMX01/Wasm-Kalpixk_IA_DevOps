...
def ensure_ensemble():
    ...
    if _ensemble is None:
        ...
        if _ensemble.autoencoder.net is None or not getattr(_ensemble.autoencoder, "is_trained", False):
            from loguru import logger
            logger.info("Auto-training baseline...")
            ...
            # Seed the drift guard with the baseline to establish initial threshold
            with torch.no_grad():
                X_tensor = torch.from_numpy(X).to(_device)
                scores, _, _, _ = _ensemble.predict(X_tensor)
                # Seed buffer and force recalibration
                _ensemble.drift_guard.update(scores, is_confirmed_benign=True)
                # Ensure the threshold is above the max score observed in baseline
