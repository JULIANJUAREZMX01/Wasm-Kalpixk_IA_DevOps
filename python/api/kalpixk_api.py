...
def ensure_ensemble():
    ...
    if _ensemble is None:
        ...
        if not getattr(_ensemble.autoencoder, "is_trained", False):
            ...
            # Calibration: Seed the drift guard with baseline normal samples
            with torch.no_grad():
                X_tensor = torch.from_numpy(X).to(_device)
                # Get ensemble scores for calibration
                # predict() returns (scores, methods, confidences, threshold)
                scores, _, _, _ = _ensemble.predict(X_tensor)

                # Force update with confirmed benign data to establish robust threshold
                _ensemble.drift_guard.update(scores, is_confirmed_benign=True, force_recalibrate=True)

                # Stability: ensure the threshold is at least 1.3x the maximum normal score observed
                max_score = float(np.max(scores))
                _ensemble.drift_guard.set_threshold(max(0.6, max_score * 1.3))
    return _ensemble
