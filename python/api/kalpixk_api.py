...
def _sanitize_error_content(val):
    if isinstance(val, float):
        return None if not math.isfinite(val) else val
    if isinstance(val, dict):
        return {k: _sanitize_error_content(v) for k, v in val.items()}
    if isinstance(val, list):
        return [_sanitize_error_content(v) for v in val]
    return val
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    sanitized = _sanitize_error_content(raw_errors)
    return JSONResponse(
        ...
        if isinstance(first, (int, float)):
            ...
            for x in v:
                if not math.isfinite(x):
                    raise ValueError(
                        "Features must contain finite numeric values (no NaN or Infinity)"
                    )
            ...
            for i, row in enumerate(v):
                ...
                for x in row:
                    if not math.isfinite(x):
                        raise ValueError(
                            f"Batch event features at index {i} must contain finite numeric values (no NaN or Infinity)"
                        )
        return v
