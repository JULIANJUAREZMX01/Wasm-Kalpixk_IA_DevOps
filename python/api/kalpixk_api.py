...
def ensure_ensemble():
    ...
    if _ensemble is None:
        ...
        # SIEMPRE entrenamos un baseline en el arranque si no hay uno persistido
        # para garantizar estabilidad en CI y ambientes nuevos.
        if not getattr(_ensemble.autoencoder, "is_trained", False):
            rng = np.random.default_rng(42)
            # 1000 muestras con baja varianza para un baseline sólido
            X = rng.normal(0.3, 0.05, (1000, 32)).clip(0, 1).astype(np.float32)
            X[:, 5] = 0.0  # off_hours = False
            X[:, 6] = 1.0  # source_is_internal = True
            _ensemble.autoencoder.fit(X, epochs=20)
            _ensemble.iso_forest.fit(X)

        # SIEMPRE calibramos el DriftGuard al inicio con datos normales
        # para evitar falsos positivos masivos por el umbral inicial de 0.5
        rng = np.random.default_rng(42)
        X_cal = rng.normal(0.3, 0.05, (200, 32)).clip(0, 1).astype(np.float32)
        X_cal[:, 5] = 0.0
        X_cal[:, 6] = 1.0

        with torch.no_grad():
            scores, _, _, _ = _ensemble.predict(torch.from_numpy(X_cal).to(_device))
            # Seed buffer with benign scores
            _ensemble.drift_guard.update(scores, is_confirmed_benign=True)
            # Force immediate recalibration (bypassing dampening for initial setup)
            orig_alpha = _ensemble.drift_guard.alpha
            _ensemble.drift_guard.alpha = 1.0
            _ensemble.drift_guard._recalibrate()
            _ensemble.drift_guard.alpha = orig_alpha
            # Add a safety margin (+0.05) to the baseline threshold
            _ensemble.drift_guard._current_threshold += 0.05
...
async def analyze_detect(request: Request, req: LogRequest, api_key: str = Depends(verify_api_key)):
    ...
    for i in range(len(scores)):
        ...
        if score > adaptive_threshold:
            total_anomalies += 1
            severity = "CRITICAL" if score > 0.8 else "HIGH" if score > 0.6 else "LOW"
            # Persist alert
            alert_data = {
