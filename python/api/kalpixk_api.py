...
def ensure_ensemble():
    ...
    if _ensemble is None:
        ...
        if not getattr(_ensemble.autoencoder, "is_trained", False):
            ...
            # Calibration: Set threshold based on reconstruction errors
            with torch.no_grad():
                X_tensor = torch.from_numpy(X).to(_device)
                errors = _ensemble.autoencoder.net.reconstruction_error(X_tensor).cpu().numpy()
                max_ae_err = float(np.max(errors))
                # Stabilize AE threshold: 99th percentile or max*1.5
                _ensemble.autoencoder._threshold = max(0.05, max_ae_err * 1.5)

                # Seed AdversarialDriftGuard with baseline scores
                # predict() will call update() but we'll force a clean recalibration
                scores, _, _, _ = _ensemble.predict(X_tensor)
                _ensemble.drift_guard.update(scores, force_recalibrate=True)

                # Final calibration for ensemble sensitivity
                max_ens_score = float(np.max(scores))
                # Ensure threshold is at least max_ens_score + padding to avoid FPs on baseline
                _ensemble.drift_guard.set_threshold(min(0.8, max_ens_score + 0.15))
    return _ensemble
