...
class LogRequest(BaseModel):
    # Support both single feature vector and batch
...
async def analyze_detect(req: LogRequest, api_key: str = Depends(verify_api_key)):
    ...
    if feats.shape[1] != 32:
        raise HTTPException(422, f"Se esperan 32 features por vector, recibidas: {feats.shape[1]}")

    if req.event_ids and len(req.event_ids) != feats.shape[0]:
        raise HTTPException(422, f"features y event_ids deben tener el mismo tamaño ({feats.shape[0]} != {len(req.event_ids)})")
