...
class AdversarialDriftGuard:
    """
    Robust sliding-window threshold against adversarial baseline shifting ('boiling frog' attacks).
    Uses Median and Median Absolute Deviation (MAD) with dampened EMA updates to remain resilient
    against high-frequency noise and adversarial drift poisoning.
    """

    def __init__(self, window_size: int = 500, k: float = 6.0, recalibrate_every: int = 100):
        self.window_size = window_size
        self.k = k
        self.recalibrate_every = recalibrate_every
    ...
    def update(
        ...
        Accepts a single float score or a list/batch of scores.
        """
    ...
    def _recalibrate(self, force: bool = False) -> None:
        ...
        mad_floored = max(0.01, mad)
        calc_thresh = median + self.k * (mad_floored * 1.4826)
        if force or self._total_updates <= len(self._buffer):
            self._current_threshold = float(calc_thresh)
        else:
            alpha = 0.1
            self._current_threshold = float((1 - alpha) * self._current_threshold + alpha * calc_thresh)
        self._updates_since_recalc = 0
