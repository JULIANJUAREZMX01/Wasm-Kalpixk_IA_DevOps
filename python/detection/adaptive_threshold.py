...
class AdversarialDriftGuard:
    """
    Robust adaptive threshold guard resistant to adversarial baseline drift ('boiling frog' attacks).
    Uses median and Median Absolute Deviation (MAD) with EMA dampening to prevent malicious baseline shifting.
    """
    ...
    def update(
        ...
        if isinstance(scores, (float, int)):
            scores_array = np.array([float(scores)])
        ...
        with self._lock:
            if is_confirmed_benign:
                ...
                valid_scores = scores_array[scores_array < self._current_threshold]

            n_new = len(valid_scores)
            if n_new > 0:
                # Use extend instead of appending one by one
                self._buffer.extend(valid_scores.tolist())
                self._updates_since_recalc += n_new
                self._total_updates += n_new
    ...
    def _recalibrate(self, force_direct: bool = False) -> None:
        ...
        mad_scaled = max(0.01, mad * 1.4826)
        target_threshold = float(median + self.k * mad_scaled)
