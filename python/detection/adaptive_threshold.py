...
class AdversarialDriftGuard:
    """
    Adversarial-resistant adaptive threshold guard using robust statistics (Median and MAD)
    with dampened EMA recalibration to defend against baseline shifting ('boiling frog') attacks.
    """
    ...
    def update(
        ...
        Accepts a single float score or a list of scores.
        """
        scores = score if isinstance(score, list) else [score]
        with self._lock:
            for s in scores:
                s_float = float(s)
                if is_confirmed_benign or s_float < self._current_threshold:
                    self._buffer.append(s_float)
                    self._updates_since_recalc += 1
                    self._total_updates += 1
    ...
    def _recalibrate(self, force_direct: bool = False) -> None:
        """Recompute threshold using robust Median/MAD. Internal use only."""
        data = np.array(self._buffer, dtype=np.float64)
        med = float(np.median(data))
        mad = float(np.median(np.abs(data - med)))
        robust_mad = max(mad * 1.4826, self.mad_floor)
        raw_threshold = float(med + self.k * robust_mad)

        if force_direct or self._current_threshold == 0.5:
