...
def test_alias_compatibility():
    # AdaptiveThreshold should be an alias of AdversarialDriftGuard
    assert AdaptiveThreshold is AdversarialDriftGuard


def test_robust_recalibration_dampening():
    ...
    # Target threshold: Median(0.1) + 3 * 1.4826 * MAD(0.1) = 0.1 + 0 = 0.1
    # Current threshold: 0.1 * 0.1 (target) + 0.9 * 0.5 (initial) = 0.01 + 0.45 = 0.46
    at.update([0.1] * 10)

    assert at.current_threshold == pytest.approx(0.46)
...
def test_batch_update():
    at = AdversarialDriftGuard(recalibrate_every=10, alpha=0.1)
    # Batch update should trigger recalibration if count reached
    at.update([0.1] * 10)
    assert at.current_threshold < 0.5
...
def test_drift_guard_thread_safety():
    ...
    def worker():
        for _ in range(10):
            at.update([np.random.uniform(0.0, 0.2) for _ in range(10)])
...
def test_is_anomaly():
    ...
    # Recalibrate (force lower)
    at.update([0.1] * 100) # Multiple cycles

    new_threshold = at.current_threshold
    assert new_threshold < 0.5
    assert at.is_anomaly(0.49)
